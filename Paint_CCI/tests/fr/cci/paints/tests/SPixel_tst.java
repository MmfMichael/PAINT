...
import fr.cci.paint.shapes.SPixel;
import junit.framework.TestCase;

class SPixel_tst extends TestCase{
	...
	void Given_DefaultPixel_When_toString_Then_ShouldReturnText(){
		SPixel p = new SPixel();
		assertEquals("Pixel : 1|1 isSelected=false", p.toString());		
	}
	...
	void Given_DefaultPixel_When_print_Then_ShouldDisplayText(){
		SPixel p = new SPixel();
		p.print();
		assertEquals("Pixel : 1|1 isSelected=false", outContent.toString().replace("\r\n", "\n"));
	}
	...
	void Given_DefaultPixelSelected_When_Print_Then_ShouldDisplayText(){
		...
		p.select();
		p.print();
		assertEquals("Pixel : 1|1 isSelected=true", outContent.toString().replace("\r\n", "\n"));
	}
	...
	void Given_DefaulPixel_When_translatingAndPrint_Then_printedShouldBe() {
		SPixel p = new SPixel();
		p.translate(20, 30);
		assertEquals("Pixel : 21|31 isSelected=false", p.toString());
		p.translate(-5, 20);
		assertEquals("Pixel : 16|51 isSelected=false", p.toString());
		p.translate(85, -50);
		assertEquals("Pixel : 101|1 isSelected=false", p.toString());
	}
